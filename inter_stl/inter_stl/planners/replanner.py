...
from inter_stl.planners.main_planner import MinimalClientAsync, plan_to_plan_msg
from inter_stl.helpers.beziers import get_derivative_control_points_gurobi
from inter_stl.helpers.read_write_plan import csv_to_plan, plan_to_csv
from inter_stl.helpers.plot_rvars_hvars import plot_rvars_hvars
...
class RePlanner(Node):
    ...
    def setup(self):
        ...
        # S is a flag to indicate if we care about end position or end velocity more, needed when braking to 0 velocity
        s = opti.parameter()
        self.params['s'] = s
        ...
        opti.subject_to(eps <= 1e1)
        opti.subject_to((1-s)*(new_v[0] * v[1] - new_v[1] * v[0]) <= eps) # cross product = 0 means they are colinear/parallel
        opti.subject_to((1-s)*(new_v[0] * v[1] - new_v[1] * v[0]) >= -eps) # cross product = 0 means they are colinear/parallel
        ...
        dq_end = dr_end/(dh_end+1e-15)  # avoid division by zero
        ...
        try:
            ...
            J += w_r * cs.sumsqr(object_end- x_end)
