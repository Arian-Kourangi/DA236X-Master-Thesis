...
class Robot:
    def __init__(self, name, scenario, mpc= 'R', run_id = 1, verbose=False):
        self.name = name
        rvars, hvars, ids, other_names = csv_to_plan(robot_name=name,scenario_name=scenario,path=f'/home/arian/repos/thesis/inter_stl/inter_stl/planners/plans/{SCENARIO}')
        ...
        rvars1,_,ids1,_ = csv_to_plan(robot_name='snap',scenario_name=scenario,path=f'/home/arian/repos/thesis/inter_stl/inter_stl/planners/plans/{SCENARIO}')
        ...
        self.replan = np.load(f'/home/arian/repos/thesis/inter_stl/inter_stl/saved_logs/{scenario}/{mpc}/{name}_replan_{run_id}.npz', allow_pickle=True)
        print(f"Loaded replan for {name}:") if verbose else None

        self.log = np.load(f'/home/arian/repos/thesis/inter_stl/inter_stl/saved_logs/{scenario}/{mpc}/{name}_{run_id}.npz', allow_pickle=True)
...
if SCENARIO not in ['test1','test2', 'hw_test3']: #These test do not have obstacles
...
if MPC == 'R':
    ...
    colors = {'pop':'black', 'crackle':'y', 'object':'red'}
    ls = {'pop':'k-', 'crackle':'y-', 'object':'r-'}
    inter_ls = {'pop':'cornflowerblue', 'crackle':'orange'}
    lw = 2
    thick_lw = 4
    ms = 15
    ...
    # Plot obstacles
    if SCENARIO not in ['test1','test2', 'hw_test3']:
        lb = obstacles[SCENARIO][0]
        ub = obstacles[SCENARIO][1]
    ...
    ax1.set_xticks(np.arange(xlim[0],xlim[1]+1,0.5))
    ax1.set_yticks(np.arange(ylim[0],ylim[1]+1,0.5))
    ax1.set_xticklabels(np.arange(xlim[0],xlim[1]+1,0.5))
    ax1.set_yticklabels(np.arange(ylim[0],ylim[1]+1,0.5))
    ax1.legend(
    frameon=False,
    handlelength=1.5,
    labelspacing=0.3
    ...
    #plt.tight_layout()
    #plt.show()
    #plt.savefig(f'/home/arian/repos/thesis/inter_stl/inter_stl/plots/{SCENARIO}_original_trajectories.pdf', bbox_inches="tight")
    ...
    ax2.set_xticks(np.arange(xlim[0],xlim[1]+1,0.5))
    ax2.set_yticks(np.arange(ylim[0],ylim[1]+1,0.5))
    ax2.set_xticklabels(np.arange(xlim[0],xlim[1]+1,0.5))
    ax2.set_yticklabels(np.arange(ylim[0],ylim[1]+1,0.5))
    ax2.legend(
    frameon=False,
    handlelength=1.5,
    labelspacing=0.3)
    ...
    #plt.tight_layout()
    #plt.show()
    #plt.savefig(f'/home/arian/repos/thesis/inter_stl/inter_stl/plots/{SCENARIO}_replanned_trajectories.pdf', bbox_inches="tight")
    ...
    ax3.set_xticks(np.arange(xlim[0],xlim[1]+1,0.5))
    ax3.set_yticks(np.arange(ylim[0],ylim[1]+1,0.5))
    ax3.set_xticklabels(np.arange(xlim[0],xlim[1]+1,0.5))
    ax3.set_yticklabels(np.arange(ylim[0],ylim[1]+1,0.5))
    ax3.legend(
    frameon=False,
    handlelength=1.5,
    labelspacing=0.3)
    ...
    ax0.set_xticks(np.arange(xlim[0],xlim[1]+1,0.5))
    ax0.set_yticks(np.arange(ylim[0],ylim[1]+1,0.5))
    ax0.set_xticklabels(np.arange(xlim[0],xlim[1]+1,0.5))
    ax0.set_yticklabels(np.arange(ylim[0],ylim[1]+1,0.5))
    ax0.legend(
    frameon=False,
    handlelength=1.5,
    labelspacing=0.3)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(f'/home/arian/repos/thesis/inter_stl/inter_stl/plots/{SCENARIO}_trajectories.pdf', bbox_inches="tight")