                ...
                for bzr in range(self.robots_nbzs[r]-1):
                    for bzo in range(self.objects_nbzs[o]-1):
                        ...
                        ############################### ADDITIONAL CONSTRAINTS TO LIMIT BEHAVIOURS #########################################
                            ...
                            #These select one force direction for each dimension
                            zy_pos =self.robots[r].zs_dir[o][bzr,bzo,0]  
                            zy_neg =self.robots[r].zs_dir[o][bzr,bzo,1]  
                            zx_pos =self.robots[r].zs_dir[o][bzr,bzo,2]  
                            zx_neg =self.robots[r].zs_dir[o][bzr,bzo,3]  
                            
                            #Cannot push diagonally, only push in one dimension and direction at a time
                            if not PUSH_DIAGONALLY:
                               
                                #To make sure that when one dimension is chosen, regardless of direction/sign, the other dimension has no force applied
                                ...
                                for cp in range(self.robots_ncp[r]-2):
                                    ...
                                    #If theres an interaction, in order for constraints on dr to properly enforce velocity constraints, dh needs to be constant during the interaction
                                    self.prog.addConstr(self.robots_dhvar[r][bzr][0,cp+1]-self.robots_dhvar[r][bzr][0,cp] <= self.bigM*(1-zs[o][bzr,bzo]), name=f"impact_dynamics_{r}_{bzr}_{o}_{bzo}_6")
                                    self.prog.addConstr(self.robots_dhvar[r][bzr][0,cp+1]-self.robots_dhvar[r][bzr][0,cp] >= -self.bigM*(1-zs[o][bzr,bzo]), name=f"impact_dynamics_{r}_{bzr}_{o}_{bzo}_7")   
...
###################################################### NOT USED #############################################################
        ...
        if SAVE_SOLUTIONS:
            ...
            for r in range(self.nrobots):
                ...
                plan_to_csv(robots_rsol,self.robots_hsol[r],self.robots[r].ids,self.robots[r].other_names,
                            ...
                            path='/home/arian/repos/thesis/inter_stl/planner/plans')
            for o in range(self.nobjects):
    ...
    def plot(self):
        ...
        fig.tight_layout()
        plt.savefig("/home/arian/repos/thesis/inter_stl/planner/figures/plot.svg")
        plt.savefig("/home/arian/repos/thesis/inter_stl/planner/figures/plot.png")
        ...
        fig_xy.tight_layout()
        plt.savefig("/home/arian/repos/thesis/inter_stl/planner/figures/xy_plane.svg")
        plt.savefig("/home/arian/repos/thesis/inter_stl/planner/figures/xy_plane.png")
    ...
    def animate(self):
        ...
        anim.save("/home/arian/repos/thesis/inter_stl/planner/figures/animation.mp4",writer="ffmpeg", fps=Neval/self.world.spec.tf)
    ...
    def plot_atmos(self):
        ...
        plt.savefig("./inter_stl/inter_stl/planner/figures/plot.svg")
        plt.savefig("./inter_stl/inter_stl/planner/figures/plot.png")

        # save to_save object
        with open('./inter_stl/inter_stl/planner/figures/to_save.pkl', 'wb') as f:
