...
class Robot:
    def __init__(self, name, scenario, run_id = 1, verbose=False):
        self.name = name
        ...
        self.replan = np.load(f'/home/arian/repos/thesis/impact_stl/impact_stl/saved_logs/{scenario}/{name}_replan_{run_id}.npz', allow_pickle=True)
        print(f"Loaded replan for {name}:") if verbose else None

        self.log = np.load(f'/home/arian/repos/thesis/impact_stl/impact_stl/saved_logs/{scenario}/{name}_{run_id}.npz', allow_pickle=True)
...
for name in robots.keys():
    robot = Robot(name, SCENARIO, verbose=Verbose, run_id=1)
    robots[name] = robot
    ...
    for i in range(len(robot.inter_sets)):
        ...
        print(f"Robot {robot.name} interaction {i} delta_V_pre: {delta_V_pre}")
        ...
        print(f"Robot {robot.name} interaction {i} delta_V_post: {delta_V_post}")
...
print(f"Final delta_V: {delta_V_final}")
print(f"Final delta_pos: {delta_pos_final}")
print(f"Final delta_T: {delta_T}")
